...
int Satellite::satellite_Dynamics( Satellite* S ) {
	...
	double error = pid.getError(S->actualRadius, S->desiredRadius); //for printing purposes and for graphing as well
	...
	{
		printf("\n Satellite state: aRadius = %.9f, aVelocity = %.9f, aAcceleration = %.9f, error = %.9f, shifter = %.9f, random = %.9f, kP = %.9f", S->actualRadius, S->actualVelocity, S->actualAcceleration, error,shifter,randomNumber, pid.kP);
		S->counter = 0;
	}
    S->time += 0.01 ;
	S->counter+=1;
